...
class _AddToCartItemState extends State<AddToCartItem> {
                              ...
                              Expanded(
                                ...
                                child: Container(
                                  ...
                                  child: CustomButton(context,
                                      ...
                                      onTap: () async {
                                        if (controller.addToCartText.value ==
                                            Strings.addToCart) {
                                ...
                                child: Container(
                                  ...
                                  child: CustomButton(context,
                                      ...
                                      text: Strings.viewCart, onTap: () async {
                                        ...
                                        var result = await CartPage("add").navigate(isAwait: true);
                                        if (result != null) {
                      ...
                      child: GestureDetector(
                          onTap: () {
                            ...
                            if (controller.totalQty.value > 0 &&
                                widget.modal.restaurantMenuExtraSection!
                                    .length >
                                    0) {
                              if (controller.totalQty.value == 1)
                                ...
                                updateCartData(
                                    (controller.totalQty.value - 1).toString(),
                                    getLastRowIdOfThisProduct());
                            } else {
                              updateCartData((controller.totalQty.value - 1).toString(), getLastRowIdOfThisProduct());
                            //  controller.totalQty.value = controller.totalQty.value - 1;

                            //  controller.totalPrice.value = int.parse(widget.modal.price!) * controller.totalQty.value;
                              print('UpdatedCalledtotalPrice' +controller.totalPrice.value.toString());



                              if (controller.totalQty.value == 0) {
                                controller.addToCartText.value =
                                    Strings.addToCart;
                                deleteParticularItem(
                                    getLastRowIdOfThisProduct());
                              }
  ...
  void clickToAddIcon() {
    if (widget.modal.restaurantMenuExtraSection!.length >
        0) {
      if (!checkSameRestaurantitem())
        showDifferenetRestItem();
      else
  ...
  Widget _horizontalListView(int index, String required) {
    ...
    return ListView.separated(
      shrinkWrap: true,
      primary: false,
      itemCount: widget.modal.restaurantMenuExtraSection![index]
          .restaurantMenuExtraItem!.length,
  ...
  showMenuExtraItemTitle(
      String required, int id, int index, String title, int flex) {
  ...
  void addExtraItemToList() async {
    ...
    for (int i = 0; i < widget.extraItemList.length; i++) {
      ...
      if (outerFlag) {
        ...
        updateCartData(getUpdatePositionId(newExtraItemList)[1], getUpdatePositionId(newExtraItemList)[0]);
    ...
    if(newExtraItemList.length>0)
    Strings.extraItemSuccess.toast();
  ...
  getTotalPriceOfParticularItem(){
   ...
   for (var f in cartBox.values.where((item) =>  item.id!.split("tax")[0] == widget.modal.id!)) {
     if (f != null) {
       print('salluCalled');
        ...
        try {
         ...
         if (extraList != null) {
           ...
           if(int.parse(f.quantity!)>1)
             totalExtraItemPrice=totalExtraItemPrice*int.parse(f.quantity!);


           subTotal=subTotal+int.parse(f.price!)*int.parse(f.quantity!)+totalExtraItemPrice;
   ...
   print('TotalParTItem'+controller.totalPrice.value.toString());
  ...
  void updateCartData(String newQuantity, String rowId) {
